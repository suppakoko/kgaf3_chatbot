...
> 방법은 **[docs/AF3_SETUP.ko.md](AF3_SETUP.ko.md)** 참조.
...
- AF3 획득·MCP 브리지 구동·호스트 바인딩 패치·`AF3_MCP_URL` 설정·검증까지의 전
  과정은 **[docs/AF3_SETUP.ko.md](AF3_SETUP.ko.md)** 와 **[docs/BRIDGE.ko.md](BRIDGE.ko.md)** 에 있다.
...
> 컨테이너 안에 있다(자세한 내용은 [docs/GRAPHRAG.ko.md](GRAPHRAG.ko.md)).
...
스크리닝을 돌리기 전에 고쳐야 할 사항을 명시한다(→ [docs/BRIDGE.ko.md](BRIDGE.ko.md)).
...
yoonjuho94/graphrag-mcp-server:1.1   # :8893 MCP SSE 서버
```
...
GraphRAG의 배포·사용·문제 해결 전반은 **[docs/GRAPHRAG.ko.md](GRAPHRAG.ko.md)** 참조.
...
GraphRAG 이미지(`yoonjuho94/graphrag-neo4j:1.0`, `yoonjuho94/graphrag-mcp-server:1.1`)는
...
- **웹 UI:** 브라우저에서 `http://localhost:5013` (또는 `APP_PORT`로 바꾼 포트).
  ...
  **[docs/USAGE.ko.md](USAGE.ko.md)** 참조.
...
**[docs/BRIDGE.ko.md](BRIDGE.ko.md)** 참조.
...
등)은 **[docs/BRIDGE.ko.md](BRIDGE.ko.md)** 에, AF3 획득·MCP 구동·`AF3_MCP_URL` 설정
전반은 **[docs/AF3_SETUP.ko.md](AF3_SETUP.ko.md)** 에 있다.
...
**[docs/GRAPHRAG.ko.md](GRAPHRAG.ko.md)** 6절 참조.
...
- **[docs/AF3_SETUP.ko.md](AF3_SETUP.ko.md)** — AF3 라이선스/가중치/DB 획득 + AF3 MCP 브리지 구동 + `AF3_MCP_URL` 설정·검증.
- **[docs/BRIDGE.ko.md](BRIDGE.ko.md)** — 외부 AF3 MCP 도달 문제(B3)와 해결책.
- **[docs/GRAPHRAG.ko.md](GRAPHRAG.ko.md)** — GraphRAG 지식그래프 스택(배포·사용·문제 해결).
- **[docs/USAGE.ko.md](USAGE.ko.md)** — 두 채팅 모드와 결과 웹 UI 사용법·예시.
