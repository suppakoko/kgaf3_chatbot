...
지점과 우회법은 **[`docs/BRIDGE.md`](BRIDGE.ko.md)** 에 따로 정리돼 있으니 함께
본다.
...
머신과 같을 수도 다를 수도 있다(원격 구성은 `docs/BRIDGE.ko.md` 참고).
...
- **`af3-bridge/af3_mcp_http.py`** — HTTP MCP 트랜스포트. 이 동봉 사본은
  ...
  패치가 이미 반영된 형태 — [`af3-bridge/README.md`](../af3-bridge/README.ko.md)
  및 [`docs/BRIDGE.md`](BRIDGE.ko.md) 참고). kgaf3-chat이 실제로 붙는 대상이다.
- **`af3-bridge/af3_mcp_server.py`** — stdio MCP 변형(참조용).
- **`af3-bridge/af3-mcp.service`** — 브리지를 상시 구동하는 systemd 유닛 예시.
- 자세한 설명·도구 목록·실행법은 **[`af3-bridge/README.md`](../af3-bridge/README.ko.md)** 참고.
...
([`af3-bridge/README.md`](../af3-bridge/README.ko.md) · [`docs/BRIDGE.md`](BRIDGE.ko.md)
...
SSH 터널)과 방화벽 설정 예시는 **[`docs/BRIDGE.md`](BRIDGE.ko.md)** 에 전부
...
넣는다. 나머지 설치 흐름은 [`docs/INSTALL.md`](INSTALL.ko.md)를 따른다.
...
우회법은 **[`docs/BRIDGE.md`](BRIDGE.ko.md)** 를, 사용법은
[`docs/USAGE.md`](USAGE.ko.md)를 참고한다.

---

관련 문서: [`docs/BRIDGE.md`](BRIDGE.ko.md) (B3 도달성·우회법),
[`af3-bridge/README.md`](../af3-bridge/README.ko.md) (브리지 파일 상세),
[`configure.md`](../configure.md) (`AF3_MCP_URL` · `AF3_OUTPUT_ROOT` ·
`AF3_MCP_AUTH_TOKEN`), [`docs/INSTALL.md`](INSTALL.ko.md) (전체 설치).
GraphRAG는 이와 완전히 별개의 내부 스택이다 → [`docs/GRAPHRAG.md`](GRAPHRAG.ko.md).
