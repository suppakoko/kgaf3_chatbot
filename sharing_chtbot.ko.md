...
> [README.ko.md](README.ko.md)와 [configure.md](configure.md), [docs/INSTALL.ko.md](docs/INSTALL.ko.md)만
>보면 된다.
...
- **GraphRAG 스택 정의(기본 활성)**: `docker-compose.yml`의 `neo4j`,
  ...
  `yoonjuho94/graphrag-mcp-server:1.1` (제3자 이미지). 지식 그래프(273,519 노드 /
...
그 이미지 자체(`yoonjuho94/graphrag-neo4j:1.0`, `yoonjuho94/graphrag-mcp-server:1.1`)는
...
> [docs/GRAPHRAG.ko.md](docs/GRAPHRAG.ko.md).
...
GPU / OpenRouter 키), 알려진 제약(AF3 도달성 → [docs/BRIDGE.ko.md](docs/BRIDGE.ko.md),
[docs/AF3_SETUP.ko.md](docs/AF3_SETUP.ko.md) 링크), AF3 가중치·KG 데이터셋 비포함 고지.
...
[ ] (GraphRAG) Docker Hub 이미지 2종
    (yoonjuho94/graphrag-neo4j:1.0, yoonjuho94/graphrag-mcp-server:1.1) 실제 pull 검증,
