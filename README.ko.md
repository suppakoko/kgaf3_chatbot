...
│   ├── MCP SSE  ─► graphrag-mcp (:8893)          │──► neo4j (지식 그래프 내장, bolt://neo4j:7687)
...
- **도달 가능한 외부 AF3 MCP 서버** — 스크리닝 모드에 필요.
  [docs/AF3_SETUP.ko.md](docs/AF3_SETUP.ko.md), [docs/BRIDGE.ko.md](docs/BRIDGE.ko.md) 참고.
- **OpenRouter API 키** — <https://openrouter.ai/keys>. 채팅과 `graphrag_query`에
  공통으로 쓰인다.
- **GraphRAG 모드**: 두 GraphRAG 이미지는 Docker Hub에서 자동으로 pull된다
  (`yoonjuho94/graphrag-neo4j:1.0`, `yoonjuho94/graphrag-mcp-server:1.1`). 빌드 불필요.
...
**[docs/INSTALL.ko.md](docs/INSTALL.ko.md)**, AF3 연결 검증이 실패하면
**[docs/BRIDGE.ko.md](docs/BRIDGE.ko.md)**를 본다.
...
- [docs/INSTALL.ko.md](docs/INSTALL.ko.md) — 상세 로컬 설치(사전 요구 사항, 단계별 절차,
  헬스 체크, nginx, 문제 해결).
- [docs/USAGE.ko.md](docs/USAGE.ko.md) — 두 채팅 모드와 결과 웹 UI 상세 사용법·예시.
- [docs/GRAPHRAG.ko.md](docs/GRAPHRAG.ko.md) — GraphRAG 지식 그래프 스택 배포·사용·문제 해결.
- [docs/AF3_SETUP.ko.md](docs/AF3_SETUP.ko.md) — AF3 라이선스/가중치/DB 확보 + AF3 MCP
  브리지 실행 + 호스트 패치 + 검증.
- [docs/BRIDGE.ko.md](docs/BRIDGE.ko.md) — 외부 AF3 MCP 도달성 확보(블로커 B3).
- [af3-bridge/README.ko.md](af3-bridge/README.ko.md) — 동봉 AF3 브리지 파일 설명(독립 실행 불가).
- [sharing_chtbot.ko.md](sharing_chtbot.ko.md) — 이 리포를 공개하는 관리자용 가이드(GitHub, ghcr.io, Zenodo DOI).
